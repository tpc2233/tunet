...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        super().__init__()
        mid_channels = max(1, mid_channels or out_channels)
...
def load_model(checkpoint_path, device):
    """Loads UNet, automatically detecting size/loss mode from checkpoint."""
    logging.info(f"Loading checkpoint from: {checkpoint_path}")
    checkpoint = torch.load(checkpoint_path, map_location=device)

    if 'args' not in checkpoint:
        raise ValueError("Checkpoint missing 'args'. Cannot determine model config.")
    train_args = checkpoint['args']  # Args used during training

    if not hasattr(train_args, 'resolution'):
        raise ValueError("Checkpoint args missing 'resolution'.")
    resolution = train_args.resolution
    logging.info(f"Checkpoint training resolution: {resolution}x{resolution}")

    default_lq_hidden_size = 64
    hq_default_bump_size = 96  # Must match training script defaults
    # Use the 'loss' argument from training checkpoint (default to 'l1')
    ckpt_loss = getattr(train_args, 'loss', 'l1')
    ckpt_hidden_size_saved = getattr(train_args, 'unet_hidden_size', default_lq_hidden_size)
    ckpt_effective_hidden_size = ckpt_hidden_size_saved
    if ckpt_loss == 'l1+lpips' and ckpt_hidden_size_saved == default_lq_hidden_size:
        ckpt_effective_hidden_size = hq_default_bump_size
    logging.info(f"Loading model config from checkpoint: Loss='{ckpt_loss}', Effective Hidden Size={ckpt_effective_hidden_size}")

    # Determine bilinear mode from checkpoint args (default True if missing)
    bilinear_train = getattr(train_args, 'bilinear', True)

    # Instantiate the UNet using the effective hidden size
    model = UNet(n_ch=3, n_cls=3, hidden_size=ckpt_effective_hidden_size, bilinear=bilinear_train)

    # Load the state dict (remove "module." prefix if necessary)
...
def process_image(model, image_path, output_path, resolution, stride, device, batch_size, transform, denormalize_fn, use_amp):
    ...
    unique_y = sorted(set(possible_y))
    unique_x = sorted(set(possible_x))
    for y in unique_y:
        for x in unique_x:
    ...
    with torch.no_grad():
        for i in range(0, num_slices, batch_size):
            ...
            batch_src_tensor = torch.stack(batch_src_tensor_list).to(device)
...
if __name__ == "__main__":
    ...
    # --- REMOVED quality and unet_hidden_size args ---
    ...
    if not (0.0 <= args.overlap_factor < 1.0):
        logging.error("overlap_factor must be [0.0, 1.0)")
        exit(1)
    ...
    logging.info(f"Inference: Resolution={resolution}, Overlap Factor={args.overlap_factor}, Stride={stride}")

    img_extensions = ['*.png', '*.jpg', '*.jpeg', '*.bmp', '*.tif', '*.tiff', '*.webp']
    input_files = [f for ext in img_extensions for f in glob(os.path.join(args.input_dir, ext))]
    if not input_files:
    ...
    logging.info(f"Inference finished. Total time: {total_end_time - total_start_time:.2f} seconds")
